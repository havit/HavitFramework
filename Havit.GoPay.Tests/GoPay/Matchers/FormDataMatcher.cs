...
public class FormDataMatcher : IMockedRequestMatcher
{
	...
	public bool Matches(System.Net.Http.HttpRequestMessage message)
	{
		...
		return values.All(matchPair => formData.Any(p => p.Key == matchPair.Key && p.Value == matchPair.Value));
	}
	...
	{
		return (httpContent?.Headers.ContentType != null) && (IsFormData(httpContent.Headers.ContentType.MediaType) || httpContent is MultipartFormDataContent);
	}
