...
public class DbDataSeedPersisterTests
{
	...
	{
		// Arrange
		foreach (var (persister, dbContext) in GetDbDataSeedPersisters())
		{
	...
	public void DbDataSeedPersister_GetPropertiesForInserting_ItemWithDefaultValue()
	{
		...
		foreach (var (persister, dbContext) in GetDbDataSeedPersisters())
		{
			...
			Assert.Contains(item => item.Name == nameof(ItemWithDefaultValue.Symbol), result);
			Assert.Contains(item => item.Name == nameof(ItemWithDefaultValue.CommentWithDefaultValue), result);
			Assert.Contains(item => item.Name == nameof(ItemWithDefaultValue.Id), result); // je volitelně generované sekvencí, budeme hodnotu nastavovat
		}
	}
	...
	{
		// Arrange
		var persister = GetDbDataSeedPersisters().First().Persister;
	...
	public void DbDataSeedPersister_GetPropertiesForInserting_EntityWithoutGeneratedKey()
	{
		...
		foreach (var (persister, dbContext) in GetDbDataSeedPersisters())
		{
			...
			Assert.Contains(item => item.Name == nameof(ItemWithoutGeneratedKey.Value), result);
			Assert.Contains(item => item.Name == nameof(ItemWithoutGeneratedKey.Id), result); // není automaticky generované, budeme hodnotu nastavovat
		}
	}
	...
	public void DbDataSeedPersister_GetPropertiesForInserting_EntityWithAutogeneratedKey()
	{
		foreach (var (persister, dbContext) in GetDbDataSeedPersisters())
		{
			...
			Assert.Contains(item => item.Name == nameof(ItemWithAutogeneratedKey.Symbol), result);
			Assert.DoesNotContain(item => item.Name == nameof(ItemWithAutogeneratedKey.Id), result);
		}
	}
	...
	public void DbDataSeedPersister_GetPropertiesForInserting_EntityWithKeyGeneratedBySequence()
	{
		...
		foreach (var (persister, dbContext) in GetDbDataSeedPersisters())
		{
			...
			// Assert
			Assert.HasCount(2, result);
			Assert.Contains(item => item.Name == nameof(ItemWithKeyGeneratedBySequence.Value), result);
			Assert.Contains(item => item.Name == nameof(ItemWithKeyGeneratedBySequence.Id), result); // je volitelně generované sekvencí, budeme hodnotu nastavovat
		}
	}
	...
	public void DbDataSeedPersister_GetPropertiesForUpdating_EntityWithoutGeneratedKey()
	{
		...
		foreach (var (persister, dbContext) in GetDbDataSeedPersisters())
		{
			...
			Assert.HasCount(1, result);
			Assert.Contains(item => item.Name == nameof(ItemWithoutGeneratedKey.Value), result);
			Assert.DoesNotContain(item => item.Name == nameof(ItemWithAutogeneratedKey.Id), result); // primární klíč nelze aktualizovat
		}
	}
	...
	public void DbDataSeedPersister_GetPropertiesForUpdating_EntityWithKeyGeneratedBySequence()
	{
		...
		foreach (var (persister, dbContext) in GetDbDataSeedPersisters())
		{
			...
			Assert.HasCount(1, result);
			Assert.Contains(item => item.Name == nameof(ItemWithKeyGeneratedBySequence.Value), result);
			Assert.DoesNotContain(item => item.Name == nameof(ItemWithKeyGeneratedBySequence.Id), result); // primární klíč nelze aktualizovat
		}
	}
	...
	// Private method, tuple possible
	private List<PersisterAndDbContext> GetDbDataSeedPersisters()
	{
		Mock<IUnitOfWork> unitOfWorkMock = new Mock<IUnitOfWork>(MockBehavior.Strict);
		unitOfWorkMock.Setup(m => m.Clear());

		var dbContextInMemory = new DbDataSeedPersisterTestsDbContext(new DbContextOptionsBuilder<DbDataSeedPersisterTestsDbContext>().UseInMemoryDatabase("FAKE").Options);
		var dbContextSqlServer = new DbDataSeedPersisterTestsDbContext(new DbContextOptionsBuilder<DbDataSeedPersisterTestsDbContext>().UseSqlServer("FAKE").Options);

		var dbDataSeedPersisterInMemory = new DbDataSeedPersister(dbContextInMemory, unitOfWorkMock.Object);
		var dbDataSeedPersisterServer = new DbDataSeedPersister(dbContextSqlServer, unitOfWorkMock.Object);

		return
		[
			new PersisterAndDbContext(dbDataSeedPersisterInMemory, dbContextInMemory),
			new PersisterAndDbContext(dbDataSeedPersisterServer, dbContextSqlServer)
		];
	}
	...
	private record PersisterAndDbContext(DbDataSeedPersister Persister, IDbContext DbContext);
