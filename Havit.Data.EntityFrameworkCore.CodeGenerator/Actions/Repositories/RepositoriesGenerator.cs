	...
	IGenericGenerator _genericGenerator) : IDataLayerGenerator
{
	public async Task GenerateAsync(CancellationToken cancellationToken)
	{
		...
		await _genericGenerator.GenerateAsync(_dbRepositoryModelSource, repositoryModel => new InterfaceRepositoryTemplate(repositoryModel), new InterfaceRepositoryFileNamingService(_dataLayerProject), OverwriteBahavior.SkipWhenAlreadyExists, cancellationToken);
		...
		// db repository
		await _genericGenerator.GenerateAsync(_dbRepositoryModelSource, repositoryModel => new DbRepositoryTemplate(repositoryModel), new DbRepositoryFileNamingService(_dataLayerProject), OverwriteBahavior.SkipWhenAlreadyExists, cancellationToken);
