...
namespace Havit.BusinessLayerTest
{
	...
	public partial class CurrencyCollectionBase : BusinessObjectCollection<Currency, CurrencyCollection>
	{
		#region Constructors
		/// <summary>
		/// Vytvoří novou instanci kolekce.
		/// </summary>
		public CurrencyCollectionBase() : base()
		{
		}
		...
		/// </summary>
		public CurrencyCollectionBase(IEnumerable<Currency> collection) : base(collection)
		{
		}
		#endregion
		...
		/// </summary>
		public override Currency Find(Predicate<Currency> match)
		{
			LoadAll();
		...
		/// </summary>
		public override CurrencyCollection FindAll(Predicate<Currency> match)
		{
			LoadAll();
		...
		public override void Sort(Comparison<Currency> comparsion)
		{
			LoadAll();
		...
		public void LoadAll(DbTransaction transaction)
		{
			...
			Dictionary<int, Currency> ghosts = new Dictionary<int, Currency>();
			
			for (int i = 0; i < this.Count; i++)
			{
				Currency currentObject = this[i];
				if ((currentObject != null) && (!currentObject.IsLoaded))
				{
					DataRecord cachedDataRecord = Currency.GetDataRecordFromCache(currentObject.ID);
			...
			if (ghosts.Count > 0)
			{
				...
				queryParams.Conditions.Add(ReferenceCondition.CreateIn(Currency.Properties.ID, ghosts.Keys.ToArray()));
				...
				using (DbDataReader reader = DbConnector.Default.ExecuteReader(dbCommand))
				{
					while (reader.Read())
					{
						...
						Currency ghost = ghosts[id];
						if (!ghost.IsLoaded)
						{
							ghost.Load(dataRecord);
							Currency.AddDataRecordToCache(ghost.ID, dataRecord);
						}
					}
				}
			}
