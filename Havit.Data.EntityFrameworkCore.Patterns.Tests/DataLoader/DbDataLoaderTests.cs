...
namespace Havit.Data.Entity.Patterns.Tests.DataLoader
{
	...
	public class DbDataLoaderTests
	{
		...
		public void DbDataLoader_LoadCollection_OneToMany_DoesNotLoadAlreadyLoaded()
		{
			...
			Mock<IDbContext> mockDbContext = new Mock<IDbContext>(MockBehavior.Strict);
			Mock<IDbSet<Child>> mockChildDbSet = new Mock<IDbSet<Child>>(MockBehavior.Strict);
			Mock<IDbSet<Master>> mockMasterDbSet = new Mock<IDbSet<Master>>(MockBehavior.Strict);

			mockDbContext.Setup(m => m.Set<Child>()).Returns(mockChildDbSet.Object);
			mockDbContext.Setup(m => m.Set<Master>()).Returns(mockMasterDbSet.Object);
