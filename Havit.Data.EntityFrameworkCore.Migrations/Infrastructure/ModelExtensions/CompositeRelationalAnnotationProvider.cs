...
{
	private readonly IEnumerable<IRelationalAnnotationProvider> _providers;
	...
	{
		return base.For(model, designTime).Concat(_providers.SelectMany(provider => provider.For(model, designTime)));
	}

	/// <inheritdoc />
	public override IEnumerable<IAnnotation> For(ITable table, bool designTime)
	{
		return base.For(table, designTime).Concat(_providers.SelectMany(provider => provider.For(table, designTime)));
	}

	/// <inheritdoc />
	public override IEnumerable<IAnnotation> For(IColumn column, bool designTime)
	{
		return base.For(column, designTime).Concat(_providers.SelectMany(provider => provider.For(column, designTime)));
	}

	/// <inheritdoc />
	public override IEnumerable<IAnnotation> For(IView view, bool designTime)
	{
		return base.For(view, designTime).Concat(_providers.SelectMany(provider => provider.For(view, designTime)));
	}
	...
	{
		return base.For(sqlQuery, designTime).Concat(_providers.SelectMany(provider => provider.For(sqlQuery, designTime)));
	}
	...
	{
		return base.For(function, designTime).Concat(_providers.SelectMany(provider => provider.For(function, designTime)));
	}
	...
	{
		return base.For(foreignKey, designTime).Concat(_providers.SelectMany(provider => provider.For(foreignKey, designTime)));
	}

	/// <inheritdoc />
	public override IEnumerable<IAnnotation> For(ITableIndex index, bool designTime)
	{
		return base.For(index, designTime).Concat(_providers.SelectMany(provider => provider.For(index, designTime)));
	}

	/// <inheritdoc />
	public override IEnumerable<IAnnotation> For(IUniqueConstraint constraint, bool designTime)
	{
		return base.For(constraint, designTime).Concat(_providers.SelectMany(provider => provider.For(constraint, designTime)));
	}

	/// <inheritdoc />
	public override IEnumerable<IAnnotation> For(ISequence sequence, bool designTime)
	{
		return base.For(sequence, designTime).Concat(_providers.SelectMany(provider => provider.For(sequence, designTime)));
	}

	/// <inheritdoc />
	public override IEnumerable<IAnnotation> For(ICheckConstraint checkConstraint, bool designTime)
	{
		return base.For(checkConstraint, designTime).Concat(_providers.SelectMany(provider => provider.For(checkConstraint, designTime)));
	}
}