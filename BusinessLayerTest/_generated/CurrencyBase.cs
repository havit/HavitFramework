...
namespace Havit.BusinessLayerTest
{
	...
	public abstract class CurrencyBase : ActiveRecordBusinessObjectBase
	{
		...
		/// <summary>
		/// Název měny. [nvarchar(50), not-null]
		/// </summary>
		...
		/// <summary>
		/// Zkratka měny, běžná, např. Kč. [nvarchar(5), not-null]
		/// </summary>
		...
		{
			string cacheKey = "Havit.BusinessLayerTest.Currency.DataRecordsCache|ID=" + id.ToString();
		...
		{
			HttpRuntime.Cache.Remove("Havit.BusinessLayerTest.Currency.GetAll|includeDeleted=True");
			HttpRuntime.Cache.Remove("Havit.BusinessLayerTest.Currency.GetAll|includeDeleted=False");
		}
		#endregion
		...
		private static CurrencyCollection GetList(DbCommand dbCommand, DataLoadPower dataLoadPower)
		{
			...
			using (DbDataReader reader = DbConnector.Default.ExecuteReader(dbCommand))
			{
				while (reader.Read())
				{
					...
					{
						AddDataRecordToCache(currency.ID, dataRecord);
					}
					result.Add(currency);
				}
			}
