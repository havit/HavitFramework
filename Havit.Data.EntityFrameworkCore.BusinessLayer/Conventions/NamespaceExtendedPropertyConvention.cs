    ...
    public class NamespaceExtendedPropertyConvention : IModelConvention
	{
		...
		public void Apply(ModelBuilder modelBuilder)
		{
			foreach (IMutableEntityType entityType in modelBuilder.Model.GetApplicationEntityTypes(includeManyToManyEntities: false))
			{
				...
				var notIgnoredProperties = entityType.GetProperties().Where(property => !property.IsShadowProperty && !property.PropertyInfo.GetCustomAttributes(typeof(IgnoredAttribute), false).Any()).ToList();
				if ((notIgnoredProperties.Count == 2) && notIgnoredProperties.All(property => property.IsPrimaryKey()) && notIgnoredProperties.All(property => property.IsForeignKey()))
				{
					continue;
				}
