...
{
	/// <summary>
	/// Vylepöen˝ <see cref="DropDownList"/>.
    /// Podporuje lepöÌ zpracov·nÌ hodnoty DataTextField p¯i databindingu.	
	/// </summary>
	/// <remarks>
	/// Known issue:
	/// Pokud je v jednom requestu nejprve nastaven SelectedIndex/SelectedValue a potÈ je proveden DataBind, pokusÌ se DataBind znovu vybrat poloûku, kter· byla vybr·na.
	/// Pokud se to nepovede (nenÌ nalezena), pak je vyhozena v˝jimka.
	/// NenÌ öance toto potlaËit (a podle RH nem·me toto chov·nÌ ruöit, p¯estoûe nem·m tuöenÌ, k Ëemu to je),
	/// proto do ClearSelection doplÚujeme vymaz·nÌ p¯Ìznak˘, kter˝mi se ¯ÌdÌ DataBind.
	/// </remarks>
	public class DropDownListExt : DropDownList
	{
		#region ItemDataBound (event)
		/// <summary>
		/// Ud·lost, kter· se vol· po vytvo¯enÌ itemu a jeho data-bindingu.
		/// </summary>
		...
		private int cachedSelectedIndex = -1; // JK: Aù mi nÏkdo z MS vysvÏtlÌ, jak m· tohle fungovat. Podle mne to ani nem· logiku.
		...
		/// <summary>
		/// ZruöÌ v˝bÏr aktu·lnÏ vybranÈ poloûky.
		/// </summary>
		...
		protected override void PerformDataBinding(IEnumerable dataSource)
		{
			...
			if (this.cachedSelectedValue != null)
			{
				...
				{
					throw new ArgumentException("Hodnoty SelectedValue a SelectedIndex se navz·jem vyluËujÌ.");
				}
		...
		/// <summary>
		/// Vytvo¯Ì ListItem, souË·st PerformDataBindingu.
		/// </summary>
		...
		/// <summary>
		/// Implementace nahrazujÌcÌ internal metody ListItemCollection.FindByValueInternal()
		/// </summary>
		/// <param name="listItemCollection">prohled·van· ListItemCollection</param>
		/// <param name="value">hledan· hodnota</param>
		/// <returns></returns>
