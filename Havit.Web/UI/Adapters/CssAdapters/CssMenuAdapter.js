		...
		while (child)
		{
			if (child.tagName == "UL") // element has other children too, let's get just menu items
			{
				...
				while (grandchild)
				{
					if (grandchild == "LI")
					{
						UnhoverRecursive__AspNetMenu(grandchild.id); // recurse
					}
					grandchild = grandchild.nextSibling;
				}			
			}
...
function SetHover__AspNetMenu()
{
    ...
    for (var i=0; i<menus.length; i++)
    {
        ...
        {			
			var disappearAfter = parseInt(menus[i].getAttribute('disappearAfter'), 10); // reads the delay rendered by adapter
            ...
            for (var k=0; k<items.length; k++)
            {
				...
				items[k].disappearAfter = disappearAfter; 
                ...
                items[k].onmouseout = function() {
					...
					element.timerID = setTimeout(new Function('Unhover__AspNetMenu(\'' + element.id + '\');'), element.disappearAfter);
				};
