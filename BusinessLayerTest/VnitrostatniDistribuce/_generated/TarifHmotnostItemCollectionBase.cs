...
namespace Havit.BusinessLayerTest.VnitrostatniDistribuce
{
	...
	public partial class TarifHmotnostItemCollectionBase : BusinessObjectCollection<TarifHmotnostItem, TarifHmotnostItemCollection>
	{
		#region Constructors
		/// <summary>
		/// Vytvoří novou instanci kolekce.
		/// </summary>
		public TarifHmotnostItemCollectionBase() : base()
		{
		}
		...
		/// </summary>
		public TarifHmotnostItemCollectionBase(IEnumerable<TarifHmotnostItem> collection) : base(collection)
		{
		}
		#endregion
		...
		/// </summary>
		public override TarifHmotnostItem Find(Predicate<TarifHmotnostItem> match)
		{
			LoadAll();
		...
		/// </summary>
		public override TarifHmotnostItemCollection FindAll(Predicate<TarifHmotnostItem> match)
		{
			LoadAll();
		...
		public override void Sort(Comparison<TarifHmotnostItem> comparsion)
		{
			LoadAll();
		...
		public void LoadAll(DbTransaction transaction)
		{
			...
			Dictionary<int, TarifHmotnostItem> ghosts = new Dictionary<int, TarifHmotnostItem>();
			
			for (int i = 0; i < this.Count; i++)
			{
				TarifHmotnostItem currentObject = this[i];
				if ((currentObject != null) && (!currentObject.IsLoaded))
				{
					DataRecord cachedDataRecord = TarifHmotnostItem.GetDataRecordFromCache(currentObject.ID);
			...
			if (ghosts.Count > 0)
			{
				...
				queryParams.Conditions.Add(ReferenceCondition.CreateIn(TarifHmotnostItem.Properties.ID, ghosts.Keys.ToArray()));
				...
				using (DbDataReader reader = DbConnector.Default.ExecuteReader(dbCommand))
				{
					while (reader.Read())
					{
						...
						TarifHmotnostItem ghost = ghosts[id];
						if (!ghost.IsLoaded)
						{
							ghost.Load(dataRecord);
							TarifHmotnostItem.AddDataRecordToCache(ghost.ID, dataRecord);
						}
					}
				}
			}
